...
const ProductDetails = () => {
  ...
  const handleCart = async (e) => {
    ...
    if (!user || !user.loggedInUser || !user.loggedInUser.user || !user.loggedInUser.user._id) {
        alert.show('User not logged in');
        return;
    }
    ...
    try {
        ...
        const itemExists = items?.some(item => item.product._id === selectedProductId);
        if (itemExists) {
            alert.show('Item already in Cart');
        } else {
            // Dispatch action to add the product to the cart
            await dispatch(addTocart({ product: selectedProductId, user: userId, quantity: 1 }));
  ...
  // const average = selectedProduct?.reviews?.average || 0;
  return (
